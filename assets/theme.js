...
class mainHeader {
  constructor(siteheader){
    ...
    setHeaderHeight(){
      document.documentElement.style.setProperty('--hdrht', `${this.hd.offsetHeight}px`);
    }
...
function checkWishlist(i){
  var btns = document.querySelectorAll(".addwishlist"),
      ...
      countMobile = document.querySelector("#wishlistItemsMobile");

  if(wItems == null) return;

  if (count) count.innerHTML = wItems.length;
  if (countMobile) countMobile.innerHTML = wItems.length;
...
class NewsletterPopup extends HTMLElement {
  ...
  onShowNewletter(){
    const donotnwpp = document.querySelector(".tingle-modal__close"),
...
/** Currency Helpers * - Accounting.js - https://openexchangerates.github.io/accounting.js/ **/
